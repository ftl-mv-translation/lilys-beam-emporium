...
script.on_internal_event(Defines.InternalEvents.PROJECTILE_FIRE, function(projectile, weapon)
    ...
    if chaoslockBeams[weapon.blueprint.name] then
        local speed = (weapon.blueprint.speed > 0) and weapon.blueprint.speed or 5
        local beamTime = weapon.blueprint.length / (speed * 16)
        local projData = {}
        projectile.table["mods.lilybeams.chromatic.lock"] = projData
        projData.lifetime = beamTime
        projData.lifetimeCurrent = beamTime
    ...
    if not check_paused() then
        for projectile in vter(Hyperspace.App.world.space.projectiles) do
            ...
            if projData then
                ...
                -- Calculate color
                local r, g, b = hue2rgb(1.0 - (30.0 / 360.0) + (60 / 360) * (projData.lifetimeCurrent / projData.lifetime))
                projectile.color.r = r * 255
                projectile.color.g = g * 255
                projectile.color.b = b * 255
            end
            projData = projectile.table["mods.lilybeams.chromatic.lock"]
            if projData then
                -- Tick timer
                projData.lifetimeCurrent = projData.lifetimeCurrent - time_increment()

                -- Calculate color
                local r, g, b = hue2rgb(0.5 + (30.0 / 360.0) - (60 / 360) * (projData.lifetimeCurrent / projData.lifetime))
                projectile.color.r = r * 255
                projectile.color.g = g * 255
                projectile.color.b = b * 255
            end
