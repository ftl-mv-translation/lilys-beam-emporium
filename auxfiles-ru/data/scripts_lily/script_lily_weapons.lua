...
local defense_types = {
    DRONES = { [3] = true, [5] = true, name = "Дроны" },
    MISSILES = { [1] = true, [2] = true, [5] = true, name = "Все твердые снаряды" },
    DRONES_MISSILES = { [1] = true, [2] = true, [3] = true, [5] = true, name = "Все твердые снаряды и дроны" },
    PROJECTILES = { [4] = true, name = "Нетвердые снаряды" },
    PROJECTILES_MISSILES = { [1] = true, [2] = true, [4] = true, [5] = true, name = "Все снаряды" },
    ALL = { [1] = true, [2] = true, [3] = true, [4] = true, [5] = true, name = "Все" },
}
...
script.on_internal_event(Defines.InternalEvents.WEAPON_RENDERBOX, function(weapon, cooldown, maxCooldown, firstLine, secondLine, thirdLine)
    if weapon.blueprint and weapon.blueprint.name == "LILY_FOCUS_ION_PHASE" then
        ...
        local l2 = (sp - 1.0) .. " пронзание"
        local l3 = (dmg + 0.0) .. " урон"
    ...
    if weapon.blueprint and weapon.blueprint.name == "LILY_BEAM_CYCLOTRON" then
        ...
        local l3 = string.format("%.0f пронзание", sp)
        local l2 = string.format("%.0f / %.0f урон", dmg, pdmg)
        --print(l2)
        --print(l3)
        return Defines.Chain.CONTINUE, firstLine, l2, l3
    end
